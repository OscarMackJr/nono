...
#
# Fork adaptation (Plan 113-07): package selectors use this fork's Phase 102
# rename (nono-proxy -> nono-sandbox-proxy, nono-cli -> nono-sandbox-cli).
# Upstream also ran a `nono-proxy --test vault_integration` suite alongside
# spiffe_integration — that test file does not exist in this fork (no Vault
# absorb has landed; out of Phase 113's scope per 113-CONTEXT.md's "Out of
# scope" list), so that invocation is dropped rather than referencing a
# nonexistent cargo test target.
#
# NOTE (113-CONTEXT.md/CLAUDE.md standing project fact): `make` is not
# installed on the Windows dev host this phase was authored on, so this
# script is executed directly (`bash scripts/spire-test.sh`) rather than via
# `make test-spiffe` there. It is exercised for real only on Linux (this
# script and `.github/workflows/spire.yml`'s CI job, or a Linux/macOS
# reviewer host with `curl`/`tar`/`cargo` and outbound access to
# github.com/spiffe/spire's release tarball).
...
run_tests() {
    ...
    echo ""
    echo "==> cargo test: nono-sandbox-proxy spiffe_integration"
    env "${env_args[@]}" \
    ...
    if [ "$(uname -s)" = "Linux" ]; then
        echo ""
        echo "==> cargo test: nono-sandbox-cli spiffe_run"
        env "${env_args[@]}" \
            cargo test -p nono-sandbox-cli --test spiffe_run -- --nocapture
    else
        echo ""
        echo "==> Skipping nono-sandbox-cli spiffe_run (macOS: binary-level sandbox tests run in CI on Linux)"
    fi
}
