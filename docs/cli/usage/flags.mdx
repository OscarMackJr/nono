...
Deny a domain through the proxy, evaluated before the allowlist. Can be specified multiple times. Composes with (never weakens) `--allow-domain`.

Unlike upstream `nono`, `--deny-domain` does NOT activate proxy mode on its own and does NOT fall back to "allow everything except these domains." A deny-only invocation — `--deny-domain` with no `--allow-domain` and no other proxy-activating flag — is rejected at parse time with a guiding error, because that shape almost always comes from a user expecting upstream's auto-activating default-allow semantics. This is a deliberate fork divergence, documented in this fork's internal ADR set.

```bash
# Allow a network profile's hosts, but deny one specific host within it
nono run --allow-cwd --network-profile developer --deny-domain telemetry.example.com -- my-agent

# Wildcard suffix: denies subdomains but not the bare apex
nono run --allow-cwd --allow-domain api.example.com --deny-domain '*.ads.example.com' -- my-agent
```

`--config` (capability manifest mode) rejects `--deny-domain`, mirroring the existing `--config` + `--allow-domain` rejection — the manifest schema has no `deny_domain` key.
...
<Note>macOS only.</Note>

Add the proxy CA to the macOS user trust store so Go-based CLI tools (which ignore the `SSL_CERT_FILE` environment variable) trust the credential proxy. The CA is shared across sessions via the Keychain and regenerates daily. Can also be set via the `NONO_TRUST_PROXY_CA` environment variable.

```bash
nono run --allow-cwd --network-profile developer --trust-proxy-ca -- my-go-tool
```

#### `--proxy-ca-validity`

Proxy CA certificate validity in days (range `1`–`365`, default: `1`). Controls how long the generated proxy CA certificate remains valid before regeneration.

```bash
nono run --allow-cwd --network-profile developer --proxy-ca-validity 7 -- my-agent
```

#### `--allow-http2`

Enable HTTP/2 ALPN negotiation on the proxy's pooled connections to upstream servers (default: off, HTTP/1.1 only). When enabled, requests to an upstream that also supports HTTP/2 are multiplexed over a single TLS connection, which reduces connection overhead for APIs driven at high request concurrency.

```bash
nono run --allow-cwd --network-profile developer --allow-http2 -- my-agent
```

<Note>
This flag affects only the proxy's **upstream** connections — it does not change what the sandboxed process is allowed to reach, and it is not a capability grant. It is additive with the profile setting: HTTP/2 is enabled if either `--allow-http2` or the profile's `network.allow_http2` is set, and there is no flag to turn it back off for a profile that opts in.
</Note>
...
Absolute path to the writable workspace. Defaults to the current directory (canonicalized) when omitted. This single value is used as both the child engine's working directory **and** the writable filesystem grant — a single source of truth for "where the agent may write".

```bash
nono run --profile always-further/claude --workspace /srv/agent/run-42 -- claude
```
...
These flags cap CPU, memory, and process resources for the sandboxed agent tree. On Windows they are kernel-enforced through Job Objects (REQUIREMENTS.md § RESL-01..RESL-03). On Linux they are kernel-enforced, fail-closed, via cgroup v2 (requires cgroup v2 + systemd delegation; the whole launch fails closed with a clear hint if delegation is absent — no limit is ever silently skipped). On macOS, enforcement is per-flag: `--cpu-percent` is rejected at parse time, `--memory` is best-effort, and `--max-processes` is kernel-enforced but UID-wide — see each flag below.

#### `--cpu-percent`

Cap the sandboxed agent tree's CPU usage as a percentage of one logical CPU (1..=99). Kernel-enforced on Windows via `CpuRateControlInformation`. On Linux, kernel-enforced, fail-closed, via cgroup v2 `cpu.max`. On macOS the flag is rejected at parse time — no per-process CPU-quota equivalent exists on macOS.
...
Cap the sandboxed agent tree's total memory. Accepts size suffixes (`512M`, `1G`, `256K`, `2T`) or raw bytes. Kernel-enforced job-wide on Windows via `JobMemoryLimit`. On Linux, kernel-enforced, fail-closed, via cgroup v2 `memory.max`. On macOS the limit is **best-effort** via `setrlimit(RLIMIT_AS, ...)`: `RLIMIT_AS` bounds virtual address space, not RSS, so a process with sparse or shared mappings can still exceed the limit in physical memory even when the call succeeds — and the `setrlimit` call itself can fail outright (observed as `EINVAL` on Apple Silicon, where dyld pre-maps virtual address space before `main()` runs), in which case the limit is **not** applied and execution continues with only a stderr warning. See REQUIREMENTS.md § RESL-02.
...
Cap the number of active processes in the sandboxed agent tree. Range: 1..=65535. Kernel-enforced on Windows via `ActiveProcessLimit`. On Linux, kernel-enforced, fail-closed, via cgroup v2 `pids.max`. On macOS, kernel-enforced via `RLIMIT_NPROC = baseline_uid_count + N`, fail-closed on `setrlimit` failure, but UID-wide rather than descendant-tree-scoped (unlike Linux) — other processes owned by the same UID share the same budget.
...
| `--deny-domain` | `NONO_DENY_DOMAIN` | `NONO_DENY_DOMAIN=telemetry.example.com` |
