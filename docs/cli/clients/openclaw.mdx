...
- **Read+write access** to `~/.local/share/openclaw` and `~/.local/state/openclaw` (OpenClaw XDG data/state; scoped to OpenClaw's own dirs so the grant never overlaps nono's protected state root `~/.local/state/nono`)
