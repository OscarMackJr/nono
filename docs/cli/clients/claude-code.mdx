...
It also explicitly denies `$HOME/.claude` and `$WORKDIR/.claude`, so a wrapped
`Bash` call cannot rewrite Claude Code's hook settings and disable this
enforcement path even when Claude Code is launched from a sensitive directory.
