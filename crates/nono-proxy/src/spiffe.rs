...
impl SpiffeJwtSource {
    ...
    pub async fn fetch_token(&self, audience: &[String]) -> Result<(Zeroizing<String>, String)> {
        ...
        if remaining < JWT_REFRESH_SECS {
            warn!(
                "SPIFFE JWT-SVID expires in {}s, below the {}s refresh threshold; check SPIRE JWT TTL configuration",
...
// `cargo fmt --all --check` rejects it. Rewritten to NESTED `if let` per hook_runtime.rs's
// established fix pattern (see 113-PATTERNS.md Landmine L1) — the let-chain was never
// ported.
