...
//!
//! These tests only run when `SPIRE_AGENT_SOCKET` is set (the
//! `.github/workflows/spire.yml` CI job sets it). Without it, both tests emit
//! a loud stderr skip marker (see the D-07 marker convention this file's two
//! tests use) and return immediately, so they are safe to include in the
//! normal test suite without requiring a running SPIRE agent (D-07,
//! 113-CONTEXT.md HARD ACCEPTANCE CRITERION — a skipped test must never be
//! mistaken for a passing one). 100% of this file's tests are
//! live-agent-gated: unlike `spiffe_integration.rs`, there is no
//! fail-closed-without-SPIRE assertion here (113-RESEARCH.md's confirmed
//! finding).
//!
//! Each test:
//!   1. Binds a mock HTTP server on a random loopback port (pure
//!      `std::net::TcpListener` — no interpreter dependency, confirmed by this
//!      file's own acceptance criteria grep).
//!   2. Writes a hermetic profile pointing the SPIFFE credential at that port.
//!   3. Runs `nono run -- sh -c 'curl ...'` through the real binary.
...
//! routes through the nono proxy, which fetches the SVID and injects the
//! header.
...
// ─── Helpers (mirrors crates/nono-cli/tests/socket_access_run.rs's shape) ────
...
fn run_nono(args: &[&str], home: &Path, cwd: &Path) -> Output {
    nono_bin()
        ...
        // Denials/prompts are not expected here, but keep the runner
        // non-interactive regardless, matching socket_access_run.rs.
...
// ─── Mock HTTP server (pure Rust, no external interpreter dependency) ────────
...
impl MockHttpServer {
    ...
    /// Blocks until the background thread has captured request headers, or
    /// `timeout` elapses.
...
fn spiffe_jwt_credential_injected_end_to_end() {
    ...
    // Credential name must be hyphen-free: it becomes MOCKAPI_BASE_URL /
    // MOCKAPI_API_KEY in the child environment. The proxy routes by URL path
    // prefix (/mockapi/...) and strips it before forwarding to the upstream.
    ...
    // curl routes $MOCKAPI_BASE_URL through HTTP_PROXY (127.0.0.1 is removed
    // from NO_PROXY because the upstream is loopback). The proxy normalises
    // the absolute URL, matches the mockapi route, fetches a JWT-SVID, and
    // injects it.
    let output = run_nono(
        &[
            "run",
...
/// Catches profile-load or SVID-fetch failures that would be silent at
/// request time.
#[test]
