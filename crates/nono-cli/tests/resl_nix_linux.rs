...
fn linux_memory_limit_oom_kills_child() {
    ...
    let output = Command::new(NONO_BIN)
        .args([
            ...
            "--memory",
            "256m",
            // `--read=<dir>` grants read+execute (the split --allow-fs-read /
            // --allow-fs-exec flags were removed; AccessMode::Read maps to
            // ReadFile|ReadDir|Execute on Linux Landlock).
            "--read=/dev",
            "--read=/usr",
            "--read=/bin",
            "--read=/lib",
