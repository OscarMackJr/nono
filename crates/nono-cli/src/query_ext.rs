...
/// `domain_endpoints` contains per-domain endpoint restrictions for the
/// `nono why --host` SC3 display path.
...
///
/// Returns `(domain, Option<path>)`. The domain is normalized to lowercase.
/// Path is `None` for bare hostnames and for root-path URLs (e.g. `https://host/`).
pub fn parse_host_input(input: &str) -> (String, Option<String>) {
...
///
/// Display approximation of proxy enforcement — diverges from
/// `CompiledEndpointRules::is_allowed` in edge cases (ad-hoc `Glob` vs
/// `GlobMatcher` compilation); diagnostic only, not access control.
///
/// Returns `true` when the rules list is empty (no scoping → all paths allowed).
...
// Only called from diagnostic/formatter.rs which is Unix-only in this fork.
