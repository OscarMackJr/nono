...
//!
//! # SC2 fork invariant
//! The upstream runtime mechanism is preserved exactly: process-group isolation
//! (`setpgid`), RAII `EnvFileGuard`, `O_EXCL` env-file creation, `is_dangerous_env_var`
//! filtering, and the `mpsc`-based timeout race pattern are all ported verbatim from
//! upstream `daa55c8`. Only the fail policy is hardened (D-02).
//!
//! # Security
//!
...
mod tests {
    ...
    // ---- End-to-end before-hook (fail-closed divergence) ----

    /// FORK DIVERGENCE TEST (D-01/D-03): before-hook that exits 1 must return
    /// Err, not Ok(Vec::new()) as upstream daa55c8 does.
    #[test]
    fn test_execute_before_hook_fail_closed() {
    ...
    /// not Ok(Vec::new()) as upstream daa55c8 does.
    #[test]
    fn test_execute_before_hook_timeout_fail_closed() {
        let (_lock, _env, _home) = isolated_home();
        ...
        let result = execute_before_hook(&hook, "test-timeout-fail-closed", Path::new("/tmp"));
    ...
    /// not Ok(()) as upstream daa55c8 does.
    #[test]
    fn test_execute_after_hook_fail_closed() {
        let (_lock, _env, _home) = isolated_home();

        let dir = TempDir::new().unwrap();
        let script = dir.path().join("fail-after.sh");
        std::fs::write(&script, "#!/bin/sh\nexit 1\n").unwrap();
        std::fs::set_permissions(&script, std::fs::Permissions::from_mode(0o755)).unwrap();
    ...
    fn test_execute_before_hook_basic() {
        ...
        // TMPDIR and CUSTOM_VAR should be exported.
