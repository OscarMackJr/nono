...
mod windows_impl {
    ...
    fn purge_nono_filters(engine: &WfpEngine) -> Vec<(SweepOutcome, String, String)> {
        ...
        let enum_status =
            unsafe { FwpmFilterCreateEnumHandle0(engine.0, &template, &mut enum_handle) };
        if enum_status != 0 {
            log_sweep_event(
                ...
                for port in &request.tcp_connect_ports {
                    specs.push(PolicyFilterSpec {
                        key: deterministic_filter_key(
                            base,
                            &format!("{}-connect-{port}", layer.label),
                        ),
                        layer_key: layer.key,
    ...
    fn activate_policy_mode(request: &WfpRuntimeActivationRequest) -> WfpRuntimeActivationResponse {
        ...
        #[cfg(target_os = "windows")]
        {
            match install_wfp_policy_filters(
                request,
                &target_program,
                &outbound_rule,
                &inbound_rule,
    ...
    mod tests {
        ...
        fn proxy_policy_filter_specs_include_loopback_permits_and_block_fallback() {
            ...
            assert!(specs.iter().any(|spec| {
                matches!(spec.action, FilterAction::Block)
                    && spec.port.is_none()
                    && !spec.loopback_only
            }));
        }
        ...
        fn startup_sweep_outcome_failed_message_is_deterministic() {
            let msg =
                format_sweep_outcome(SweepOutcome::Failed, "nono-test-filter", "recv-accept-v6");
