...
impl<'a> DiagnosticFormatter<'a> {
    ...
    fn format_exit_explanation(&self, exit_code: i32) -> Vec<String> {
        ...
        match exit_code {
            ...
            code if (129..=192).contains(&code) => {
                ...
                // On Windows this code path is unreachable in practice.
                #[cfg(target_os = "linux")]
                let sigsys: i32 = 31; // SIGSYS on Linux
                #[cfg(target_os = "macos")]
                let sigsys: i32 = 12; // SIGSYS on macOS
                #[cfg(target_os = "windows")]
                let sigsys: i32 = 31; // Unused on Windows; matches Linux convention
    ...
    fn format_likely_sandbox_from_diagnostic(
        ...
        if let Some(ref remediation) = diagnostic.remediation {
            if let Some(flag) = crate::query_ext::suggested_flag_for_remediation(remediation) {
    ...
    fn canonical_for_diagnostic_path(&self, path: &Path) -> PathBuf {
        if let Some(index) = self.denials.iter().position(|denial| denial.path == path) {
            if let Some(canonical) = self.canonical_denial_paths.get(index) {
...
mod tests {
    ...
    fn test_exit_sigsys_platform_correct() {
        let caps = make_test_caps();
        let formatter = DiagnosticFormatter::new(&caps);
        // SIGSYS is 31 on Linux, 12 on macOS; use platform constant
        #[cfg(target_os = "linux")]
        let sigsys_code: i32 = 31;
        #[cfg(target_os = "macos")]
        let sigsys_code: i32 = 12;
        #[cfg(target_os = "windows")]
        let sigsys_code: i32 = 31; // Unused on Windows; test uses Linux convention
        let output = formatter.format_footer(128 + sigsys_code);
