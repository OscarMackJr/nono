...
impl CapabilitySetExt for CapabilitySet {
    ...
    fn from_profile(profile: &Profile, workdir: &Path, args: &SandboxArgs) -> Result<PreparedCaps> {
        ...
        // once the proxy has a real port. Per upstream 72bcfd66 (#1225): remove
        // the ProxyOnly { port: 0 } placeholder; NetworkIntent carries the intent.
...
fn apply_cli_network_mode(caps: &mut CapabilitySet, args: &SandboxArgs) {
    ...
    // Upstream 72bcfd66 (#1225): removed ProxyOnly { port: 0 } placeholder.
    // Proxy mode is set by start_proxy_runtime once the proxy has a real port.
}
