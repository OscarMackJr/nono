...
/// Resolve `$XDG_STATE_HOME/nono` (default `~/.local/state/nono`) on Unix,
/// or `%LOCALAPPDATA%\nono` on Windows (D-02: Windows arm).
///
/// On Windows, `dirs::state_dir()` returns `None` (XDG state is Linux-only),
/// so the Windows path maps explicitly to `%LOCALAPPDATA%\nono` — consistent
/// with the v3.0 provisioner which uses `%LOCALAPPDATA%\nono\workspace`.
