    ...
    for name in service_names {
        ...
        if let Some(cred) = custom_credentials.get(name) {
            ...
            routes.push(RouteConfig {
                ...
                // D-01 (Phase 115): `inject_mode`/`inject_header` are now
                // `Option<T>` on `CustomCredentialDef` so a `platform_overrides`
                // override that omits them inherits the base's value at merge
                // time (`merge_custom_credential_def`) instead of resetting to
                // header/`Authorization`. `RouteConfig`'s own fields stay
                // non-`Option`, so resolve the default here.
                inject_mode: cred.inject_mode.clone().unwrap_or_default(),
                inject_header: cred
                    .inject_header
                    .clone()
    ...
    fn test_resolve_credentials_custom_overrides_builtin() {
        ...
        custom.insert(
            ...
            CustomCredentialDef {
                ...
                auth: None,
                inject_mode: Some(InjectMode::Header),
                inject_header: Some("X-Custom-Auth".to_string()),
