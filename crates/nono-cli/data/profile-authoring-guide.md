...
| `no_proxy`              | array of string                   | `[]`     | Additional client-side proxy bypass entries appended to the generated `NO_PROXY`/`no_proxy` env var. Does **not** grant network access — direct connections still require matching sandbox permissions. An entry overlapping `allow_domain` (directly, via `extends` inheritance, or via `allow_domain` group-name expansion) is rejected at parse/launch time (D-06): it would silently defeat proxy filtering for a host the profile explicitly allowlisted. |
