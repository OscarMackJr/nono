    ...
    fn try_from(manifest: &CapabilityManifest) -> Result<Self> {
        ...
        if let Some(ref net) = manifest.network {
            ...
            if let Some(ref ports) = net.ports {
                ...
                // Localhost IPC port ranges. This is a separate, independent
                // pathway from the profile pathway in capability_ext.rs — a
                // manifest-driven `--config` launch never goes through
                // profile_runtime.rs, so it needs its own start<=end and
                // macOS cumulative-cap validation here.
                ...
                for &[start, end] in &ports.localhost_range {
                    ...
                    let (start, end) = match (u16::try_from(start_u), u16::try_from(end_u)) {
                        ...
                        _ => {
                            return Err(NonoError::ConfigParse(format!(
                                "localhost_range entry [{start_u}, {end_u}] is invalid: ports \
                                 must be in 1-65535"
                            )))
                        }
                    };
                    if start > end {
                        return Err(NonoError::ConfigParse(format!(
                            "localhost_range entry [{start}, {end}] is invalid: start must be \
                             <= end"
                        )));
                    }
                ...
                #[cfg(target_os = "macos")]
                {
                    ...
                    if total > crate::capability::MACOS_PORT_RANGE_LIMIT {
                        return Err(NonoError::ConfigParse(format!(
                            "localhost_range entries expand to {} unique ports, which exceeds \
                             the macOS limit of {} (sandbox_init crashes above ~17,770 rules); \
                             use smaller or fewer ranges",
                            total,
