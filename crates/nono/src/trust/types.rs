...
///
/// Files lacking this field are not nono policies and should be skipped
/// rather than rejected, to avoid false errors on identically-named foreign
/// files (e.g. AWS IAM trust policies, which use the same `trust-policy.json`
/// filename).
...
pub struct TrustPolicy {
    ...
    /// field are treated as non-nono JSON and skipped by the CLI's
    /// peek-before-full-parse loader (`nono-cli`'s `load_nono_policy`).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub predicate: Option<String>,
    /// Deprecated: versioning is encoded in the `predicate` URI.
    ///
    /// Parsed from existing policy files for backward compatibility but has
    /// no runtime effect. Will be removed in v1.0.0.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
impl TrustPolicy {
    ...
    pub fn validate_version(&self) -> Result<()> {
        if let Some(predicate) = &self.predicate {
            if predicate != TRUST_POLICY_PREDICATE {
