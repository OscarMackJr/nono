...
impl<'a> DiagnosticFormatter<'a> {
    ...
    /// Set non-filesystem sandbox operation names suppressed from the diagnostic
    /// footer (e.g. `forbidden-exec-sugid`, `mach-lookup`). The sandbox still
    /// enforces these denials; suppression only affects reporting.
    ///
    ...
    fn canonical_for_denial<'b>(&'b self, denial: &DenialRecord) -> std::borrow::Cow<'b, Path> {
        ...
        std::borrow::Cow::Owned(try_canonicalize(&denial.path))
    }
...
mod tests {
    ...
    fn suppressed_denial_annotated_with_save_skipped() {
        ...
        let suppressed = try_canonicalize(&denied);
