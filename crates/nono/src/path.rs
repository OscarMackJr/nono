...
mod tests {
    ...
    fn existing_path_canonicalizes() {
        ...
        let canonical = dir.path().canonicalize().expect("test setup");
    ...
    fn nonexistent_leaf_uses_ancestor() {
        let dir = tempfile::tempdir().expect("test setup");
        let canonical_dir = dir.path().canonicalize().expect("test setup");
    ...
    fn existing_symlink_resolves_through() {
        ...
        #[cfg(unix)]
        {
            ...
            assert_eq!(result, real_file.canonicalize().expect("test setup"));
        }
    }
}
