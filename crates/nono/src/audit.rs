...
/// approval backend was consulted (`BeforePrompt`) or AFTER approval was
/// granted but the per-kind broker helper failed (`AfterPrompt`).
/// Stage asymmetry is a permanent design property — see Phase 29 decisions.
///
/// This is a fork-local extension to the upstream `AuditEventPayload::CapabilityDecision`
/// variant. It does not exist in the upstream `always-further/nono` crate.
#[derive(Clone, Copy, Debug, Serialize, Deserialize, PartialEq, Eq)]
#[serde(rename_all = "kebab-case")]
pub enum RejectStage {
    /// Rejected before the approval backend was consulted (e.g., mask gate).
    BeforePrompt,
    /// Rejected after approval, during OS handle setup (e.g., pipe direction check).
    AfterPrompt,
}
...
pub enum AuditEventPayload {
    ...
    CapabilityDecision {
        ...
        entry: AuditEntry,
        /// Windows-AIPC-specific reject-stage marker (Phase 23 D-02, fork extension).
        /// `None` for Approved decisions, for non-Windows ledger entries,
        /// and for pre-stage rejections. `Some(BeforePrompt)` when the mask gate
        /// denies before the approval backend. `Some(AfterPrompt)` when the
        /// broker-failure flip denies after approval.
        #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct AuditVerificationResult {
    ...
    /// Fork extension: whether stored Merkle root matches recomputed Merkle root.
    /// True if no stored Merkle root was provided (vacuously true — nothing to mismatch).
...
impl AuditAttestationVerificationResult {
    /// Returns true when all attestation integrity checks passed.
    #[must_use]
    pub fn is_valid(&self) -> bool {
        self.present
            && self.key_id_matches
...
impl AuditRecorder {
    ...
    /// Record a capability approval decision with an optional Windows reject-stage marker.
    ///
    /// Fork extension: `reject_stage` carries the Windows-AIPC phase 23 D-02 discriminator.
    /// On non-Windows platforms this should always be `None`.
    pub fn record_capability_decision_with_reject_stage(
        &mut self,
        entry: AuditEntry,
        reject_stage: Option<RejectStage>,
    ) -> Result<()> {
...
pub fn verify_audit_attestation_bundle(
    ...
    if let Some(expected_public_key) = expected_public_key {
        if expected_public_key != public_key_der.as_slice() {
            return Ok(attestation_failure(
                summary,
                Some(false),
...
pub fn verify_audit_log(
    ...
    // Fork-extension: derive chain_head_matches and merkle_root_matches for display in audit_commands.
    let chain_head_matches = stored_chain_head
        .map(|stored| Some(stored) == computed_chain_head)
        .unwrap_or(true);
    let merkle_root_matches = stored_merkle_root
        .map(|stored| Some(stored) == computed_merkle_root)
        .unwrap_or(true);
...
mod tests {
    ...
    fn sample_metadata(id: &str) -> SessionMetadata {
        SessionMetadata {
            ...
            // Fork extension: RollbackStatus not in upstream struct, defaults to Available.
