...
- ⚠ **SHELL-01** — `nono shell` on Windows: Phase 30 ProcMon evidence localized failure to CSRSS console-subsystem ALPC denial during KernelBase.dll DllMain at Low-IL when the child receives a fresh console. Broker-process pattern (option 6b) PoC ran 2026-05-08 and **PASSED** — Low-IL PowerShell child inherits Medium-IL broker's console, KernelBase skips CSRSS attach, child survives DllMain and exhibits correct mandatory-label NO_WRITE_UP enforcement (`AppData\Roaming` writes denied). RESEARCH.md Assumption A1 empirically validated. Promoted from v3.0 deferral to **Phase 31 candidate** — broker-process implementation de-risked; ~7 day effort to lift PoC into production `nono shell` path
...
---
*Last updated: 2026-05-08 after Phase 30 close + same-day broker-process PoC validation. Phase 30 (Windows nono shell architecture) shipped as a failure-mode finding per CONTEXT.md D-04 timebox: Wave 2 ProcMon investigation localized the silent-launch failure to `STATUS_DLL_INIT_FAILED` (0xC0000142) at the CSRSS console-subsystem ALPC handshake during KernelBase.dll DllMain at Low-IL on the **direct** CreateProcessAsUserW path (RESEARCH Pitfall 2 — Microsoft-documented integrity-mismatch — confirmed in field). **Same-day follow-up:** quick-task `260508-lqh` produced a 370-line scoping research; quick-task `260508-m99` shipped a 196-line PoC binary that executes the broker-process pattern (AllocConsole + DuplicateTokenEx[SecurityAnonymous,TokenPrimary] + SetTokenInformation[Low] + CreateProcessAsUserW[dwCreationFlags=0]). User ran on Windows test box: child PID matches `$PID` from spawned shell, `whoami /groups` confirms `Low Mandatory Level S-1-16-4096`, child ran interactively to clean exit (`[POC] Child exit code: 0x00000000`), PSReadLine `Access denied` on `AppData\Roaming` write confirmed mandatory-label NO_WRITE_UP enforcement is firing. **RESEARCH.md A1 empirically validated.** SHELL-01 row revised from `✘ deferred to v3.0` to `⚠ Phase 31 candidate` — the broker-process pattern (option 6b from Phase 30 sixth-option analysis) is now de-risked; Phase 31 effort estimate ~7 days (was 7-9; PoC step complete). Cookbook v3.0-deferral text stays accurate for current `nono shell --profile claude-code` users until Phase 31 ships the production broker; that's when the cookbook flips. Wave 1 cascade arm code (`WindowsTokenArm::LowIlPrimary` enum + `select_windows_token_arm` helper + `pty_token_gate_tests` 6/6 + Windows-only `low_integrity_primary_token_sets_low_il` test) preserved in tree as guard for Phase 31's lift. Debug session at `.planning/debug/resolved/nono-shell-status-dll-init-failed.md` includes both the original failure-mode `## Resolution` section AND a `## Postscript: broker-pattern PoC validated A1 same-day (2026-05-08)` section. Code review (30-REVIEW.md) applied 1 hygiene fix (CR-01: `SecurityImpersonation` → `SecurityAnonymous` in `create_low_integrity_primary_token`); 4 WARNINGs + 2 INFOs documented and inherited by Phase 31 harness rework. Cumulative v2.3 progress: 11/17 requirements closed (REQ-AIPC-NIX-01 / REQ-AUDC-01..03 / REQ-WRU-01..02 / REQ-PKGS-02..03 / REQ-NTH-01..02). Phase 30 had no formal REQ-IDs (decision-coverage gate via D-01..D-10; all 10 D's addressed per 30-VERIFICATION.md). Remaining v2.3: REQ-RESL-NIX-01..03 (Phase 25 Plan 25-01, queued for Linux/macOS host), REQ-PKGS-01 + REQ-PKGS-04 (Phase 26 Plan 26-02), REQ-AAH-01 (Phase 27 — REQ-NTH-03 partial closes by proxy after v2.4-FU-1 + FU-2 land), SHELL-01 (Phase 31 — broker-process implementation, ~7 days). Phase 30 is the v2.3 milestone's final shipped phase; Phase 31 scoping is now ready (`/gsd-phase add 31` + `/gsd-discuss-phase 31` with PoC + RESEARCH as locked inputs).*
