...
## Milestone: v3.3 — UPST10 Upstream Sync (v0.64→v0.65.1) + First Real Release

**Shipped:** 2026-06-26
**Phases:** 4 (94-97) | **Plans:** 16 | **Tasks:** 22 | **Commits:** 91 | **Source diff:** 29 files, +6,179/−423

### What Was Built
Synced the fork to relocated upstream `nolabs-ai/nono` v0.65.1 and made the workspace genuinely releasable for the first time (prior v3.0/3.1/3.2 were marker-only). Phase 94 authored the `v0.64.0..v0.65.1` DIVERGENCE-LEDGER (8 commits / 4 clusters) and repointed the upstream remote. Phase 95 absorbed the will-sync clusters (Cluster A AF_UNIX mediation-deadlock fix via pidfd IPC; Cluster B additive audit + `CompiledEndpointPolicy` shared-surface; Cluster C a structural no-op preserving the Phase 89 fail-secure proxy divergence), then a gap-closure pass restored four fork-invariant regressions the cherry-pick conflict introduced. Phase 96 stood up a **local** cross-target clippy toolchain — `cross` (Docker) for linux-gnu and `cargo-zigbuild` (zig) for apple-darwin, both GREEN locally — retiring the chronic PARTIAL→CI default. Phase 97 leapfrogged the tree to **0.66.0** (first SemVer > upstream 0.65.1) across 6 crates + 6 path-dep pins + both binding repos, then built the prepare-only release pipeline: signed-order local MSI build, a 3-registry dry-run orchestrator (cargo / maturin+twine / npm), an auto-discovered `release-readiness` verify-dark gate, and an operator one-step-push runbook.

### What Worked
- **The local cross-target toolchain paid for itself on the first run.** Phase 96's `cross clippy` immediately surfaced *compile* errors (not lints) in cfg(linux) code: the Phase 95 absorb of upstream `ae77d198` had silently dropped two fork invariants (SEC-01 AF_UNIX no-grant EPERM filter + cgroup v2 enforcement). Windows-host clippy is structurally blind to that branch — exactly the blind-spot the milestone existed to close. The gate caught a real regression on day one and retired the PARTIAL→CI default that had hidden this class for many milestones ([[feedback_clippy_cross_target]]).
- **Layered verification caught regressions the executors' self-checks missed — again.** Phase 95's verifier + code review found four real fork-invariant regressions from a 120-min cherry-pick conflict resolution (arch-portable msghdr `offset_of!`, GPU Landlock enforcement, post-fork fork-safety, `endpoint_policy.evaluate()` wiring). A post-gap code-review pass then caught a Critical fail-open in the CR-01 fix itself (matched only `Deny`, forwarding `Approve` with the credential). The independent goal-backward verify remains the load-bearing gate, not the executor self-check (3rd milestone running: v3.0, v3.2, v3.3).
- **Sequential/no-worktree was the right call once more.** Phase 97-01 edits the sibling `nono-py`/`nono-ts` repos via relative paths that `isolation="worktree"` would misresolve, and Windows worktree fragility persists. Running the whole phase sequentially on the main tree avoided false isolation entirely.
- **The dry-run orchestrator surfaced a real downstream blocker before any push.** `release-dry-run.ps1` found nono-py won't `maturin build` — its `RouteConfig` is missing the `endpoint_policy` field added upstream in Phase 95. A prepare-only pipeline caught a release-stopping bug while it was still cheap to fix.

### What Was Inefficient
- **SUMMARY one-liner frontmatter empty for the 5th milestone running.** The SDK's auto-extracted MILESTONES accomplishments came through as `One-liner:` placeholders (plus one garbage `1. [Rule 3 - Blocking]…` line) for 7 of 16 plans; the entry was rewritten by hand again (v2.13/v3.0/v3.1/v3.2/v3.3). This is now a guaranteed manual close step — the executors are not populating the `one_liner:` SUMMARY field.
- **The same upstream field-drift bug (RouteConfig `endpoint_policy`) hit two repos asymmetrically.** Phase 95 added the field to the core crate but the sibling nono-py mirror wasn't updated in lockstep, so it only surfaced two phases later at dry-run. Cross-repo struct mirrors need a same-phase update checklist.
- **A publish-set ambiguity rode all the way to verification.** Three artifacts (release.yml, dry-run, runbook) disagreed on 3-vs-4 crates because `nono-shell-broker` lacked an explicit `publish=false`. Resolved cleanly at close, but the planner could have pinned the publish set as one source of truth up front.

### Patterns Established
- **A local cross-compile gate is worth standing up even on a single-OS dev host.** The Docker-`cross` + zig-`cargo-zigbuild` pair makes both Unix clippy targets locally runnable from Windows with no SDK-licensing wall (zig's bundled macOS C target satisfied aws-lc-sys/ring). Invocation nuance: direct-binary `cargo-zigbuild clippy`, NOT `cargo zigbuild clippy` (argv mis-parse). PARTIAL→CI is now a documented-runner-failure fallback, not the default.
- **Cross-repo struct mirrors need a same-phase lockstep checklist.** When an absorb changes a struct the sibling binding repo re-declares (RouteConfig), update both in the same phase or the gap surfaces at release time.
- **Pin the publish set as a single source of truth at plan time.** `publish=false` (or its absence) on every workspace crate is a release-correctness invariant the readiness gate should assert — decide it once, not at verification.

### Key Lessons
1. **Stand up the local cross-target gate before the sync, not after.** The dropped SEC-01 filter lived undetected from Phase 95 absorb until Phase 96's first local run; had the gate existed during 95 it would have failed the cherry-pick immediately.
2. **Prepare-only release engineering earns its keep by catching blockers cheaply** — the nono-py maturin failure and the publish-set divergence were both found and triaged with zero registry risk.
3. **The independent goal-backward verifier remains mandatory** — it (and the code-review pass) caught five real defects across Phases 95/97 that green executor SUMMARYs reported as done.
4. **Populate the SUMMARY `one_liner:` field** — five milestones of hand-rewriting the MILESTONES entry is a fixable executor-prompt gap.

### Cost Observations
- Not instrumented. Executor model = sonnet; orchestrator = opus. Most overhead went to the Phase 95 gap-closure cycle (restoring four regressions + the CR-01 fail-open fix) and the Phase 97 local dry-run/MSI builds, not wave execution. Sequential/no-worktree across the core + two sibling repos again avoided Windows worktree fragility.

---

## Milestone: v3.4 — UPST11 Upstream Sync to v0.66.0 + Release-Reconcile

**Shipped:** 2026-07-02
**Phases:** 3 (98-100) | **Plans:** 16 | **Tasks:** 25 | **Commits:** 89 (since v3.3 tag) | **Source diff:** 162 files, +13,726/−406

### What Was Built
Synced the fork to upstream `nolabs-ai/nono` **v0.66.0** and turned the v3.3 prepare-only pipeline into an operator-push-ready `0.66.1` release. Phase 98 refined the pre-authored `260629-toe` per-PR ledger to per-commit resolution (14 substantive commits / 8 clusters A–H + 6 noise) and settled the headline #1225 `NetworkIntent`-vs-`ProxyOnly` refactor via **ADR-98 full-sync-adopt** — grounded in actual-diff facts that it is CLI-side only (core `NetworkMode::ProxyOnly` untouched → ADR-86 boundary safe; Windows enforcement reads the derived `WindowsNetworkPolicyMode`, not the CLI intent type → WFP/AppContainer safe). Phase 99 absorbed every will-sync cluster DCO-signed (Cluster A NetworkIntent + flag-guard; Cluster C split HTTP/2 pool + fail-fast endpoint + wildcard route; D 9P warning; E org-ref; F sigstore-trust-root 0.9.0; G proxy docs/token) with tool-sandbox Cluster B won't-sync; both cross-target clippy gates GREEN locally, `make ci` clean, code-review + verifier + human sign-off APPROVED. Phase 100 leapfrogged the full 6-member workspace + both binding repos to `0.66.1`, reconciled upstream CI #1245/#1251 via **ADR-100 ADAPT-not-adopt**, closed the carried-forward nono-py `RouteConfig` PyPI blocker, and re-greened both mandatory pre-push gates + RELEASE-RUNBOOK.md. Release scope = PREPARE ONLY.

### What Worked
- **The local cross-target gate earned its keep again — second milestone running.** Phase 99's `cross clippy` (linux-gnu) caught a `let_chains` edition-2021 incompatibility in `linux.rs` that the Windows host compiler never sees, and fmt-check caught rustfmt drift in 6 files. The gate that v3.3 stood up is now routine insurance against cfg(unix) blindness ([[feedback_clippy_cross_target]]).
- **ADR-first for the high-conflict item kept the absorb clean.** Deferring the #1225 disposition to a Phase 98 ADR (mirroring v3.1/v3.3) meant Phase 99 executed against a settled, actual-diff-grounded decision rather than discovering the ProxyOnly/NetworkIntent conflict mid-cherry-pick. The audit correctly re-confirmed it CLI-side only, so the core library and Windows backends were never at risk.
- **`maturin build`, not grep, is the only thing that catches binding-repo struct drift.** The nono-py PyPI blocker turned out to be TWO missing fields, not one: the known `RouteConfig.endpoint_policy` AND a second same-class `ProxyConfig.enable_h2` that only surfaced when the build actually ran. Static inspection would have shipped a still-broken wheel. Now a durable rule: re-run maturin/napi build in the binding repos after any absorb touching nono-proxy structs ([[project_v34_opened]]).
- **Re-running the release gate after the version bump caught two latent gate bugs.** The 0.66.1 bump exposed a hardcoded `0.66.0` literal in release-readiness assertion (e) that would have false-FAILed the very re-green it exists to deliver, and a false-positive twine-detection bug in release-dry-run.ps1 (matched the substring "twine" in Python's "No module named twine" error instead of checking `$LASTEXITCODE`). A prepare-only pipeline surfaced both with zero registry risk.

### What Was Inefficient
- **The nono-py `RouteConfig` blocker was a v3.3 carry-forward that should have closed in v3.3.** It was found at v3.3's dry-run (Phase 97) but deferred as an operator carry-forward; it then blocked v3.4's release and had to be fixed here — plus its hidden `enable_h2` sibling. The cross-repo struct-mirror lockstep-checklist lesson from v3.3 was recorded but not yet operationalized as a same-phase gate.
- **Both binding repos carried stale uncommitted `0.66.0` edits from a prior dropped session.** Phase 100-03 found nono-py and nono-ts already dirty with a half-done bump from an earlier executor drop; the plan finished them in place rather than from a clean HEAD. An executor drop mid-milestone left working-tree state that a fresh plan had to reconcile rather than author.
- **SUMMARY `one_liner:` frontmatter empty for the 6th milestone running.** The SDK auto-extracted accomplishments again came through as `One-liner:` placeholders + bare fragments (`NetworkIntent enum`, `sandbox_prepare.rs:`) for ~half the plans; the MILESTONES entry was rewritten by hand yet again. This is now a permanent manual close step across v2.13/v3.0/v3.1/v3.2/v3.3/v3.4 — the executor prompt is simply not populating the field.

### Patterns Established
- **Binding-repo struct drift is invisible to grep — only the real build (`maturin build` / `napi build`) catches it.** After every upstream absorb that touches a nono-proxy struct re-declared in nono-py/nono-ts, re-run the actual binding build in each repo before calling the release ready. Same-class fields (`enable_h2` alongside `endpoint_policy`) travel together and only the build enumerates them.
- **Re-run every release gate after a version bump, not just once at plan time.** Version constants get hardcoded into gate assertions; the bump is exactly when a stale literal false-FAILs. Reference `$targetVersion`, never a baked-in string.
- **Fold-without-resolve is a legitimate milestone-close disposition for host-gated todos.** The `msi-vcredist-prereq` + `poc-cert-broker-clean-host` todos were formally documented with their SKIP_HOST_UNAVAILABLE / externally-blocked verdicts at close without claiming false completion (mirrors v3.1 Phase 90 DRAIN) — a todo can be honestly parked, not just closed or ignored.

### Key Lessons
1. **Operationalize the cross-repo struct-mirror checklist as a same-phase gate, not a recorded lesson.** Two consecutive milestones (v3.3 found it, v3.4 paid for it) is enough signal: an absorb that changes a nono-proxy struct must update + build the binding repos in the same phase.
2. **The independent verify/code-review layer remains load-bearing — 5th milestone running.** Phase 99's human sign-off + code-review + verifier confirmed no Windows-backend or ADR-86 boundary regression across the NetworkIntent absorb; the gate stack is the reason a CLI-side refactor could be adopted with confidence.
3. **Prepare-only release engineering keeps catching release-stoppers cheaply** — the double binding-field blocker and both gate bugs were all found and fixed with zero registry exposure.
4. **Populate the SUMMARY `one_liner:` field** — six milestones of hand-rewriting the MILESTONES entry is a fixable executor-prompt gap that has now outlived every intention to fix it.

### Cost Observations
- Not instrumented. Executor model = sonnet; orchestrator = opus. Most overhead went to the Phase 99 cross-target gate cycle (let_chains + fmt fixes) and the Phase 100 binding-repo reconciliation (two dirty repos + the double PyPI-field blocker), not wave execution. Sequential/no-worktree across the core + two sibling repos once more avoided Windows worktree fragility.
