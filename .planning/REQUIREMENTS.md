...
- [ ] **SIGN-03**: The **"Trusted Signing Smoke Test"** workflow runs GREEN on GitHub's clean `windows-latest` runner (Gate 1) — a throwaway exe signs and verifies `Valid` with an issuer chaining to a public `Microsoft ID Verified CS EOC/AOC CA NN` root (not `PublicTrustTest`, not the POC root) — proving the signing path is live end-to-end before any release is cut. **BLOCKED/DEFERRED 2026-07-02, corrected same day**: the GitHub Trusted Signing config (OIDC FIC + `TRUSTED_SIGNING_ACCOUNT`/`_PROFILE`/`_ENDPOINT` repo-scoped variables) **does exist and works** — a 2026-06-04 provisioning, confirmed live by independent verification and previously exercised successfully by run `28469674206` (OIDC+Sign GREEN). The sole remaining gates are: Plan 02's hardened workflow is unpushed (all Phase 101 commits local-only), pushing + dispatching is outward-facing and operator-gated under this fork's prepare-only posture, and the prior run's Verify step `UnknownError` remains unresolved. Corrected reasoning + hand-off to Phase 104 recorded in `101-SIGN03-SMOKE-VERDICT.md`.
