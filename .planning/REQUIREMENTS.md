...
- [x] **RCPT-02** *(shipped mechanism DIVERGES from this text — see status table)*: The receipt is tamper-evident on the same terms as the existing audit chain (HMAC-chained `SecurityEventLayer`), so a downstream governance consumer can verify a receipt was not edited after the fact.
...
| RCPT-02 | Phase 118 | **Complete AS BUILT, but the requirement TEXT above is STALE — operator decision pending.** Text says "HMAC-chained `SecurityEventLayer`"; a KEYLESS SHA-256 chain shipped (D-25), because `SecurityEventLayer`'s key is per-process OsRng zeroized on Drop and would make receipts unverifiable after process exit. The parenthetical was also inaccurate about the pre-existing codebase: `audit.rs`, the actual "existing audit chain", is itself keyless. Tamper-EVIDENCE and ordering hold; authorship is explicitly NOT claimed. Accepted limitation: a clean tail-record deletion is undetectable by a keyless chain with no external anchor, and `nono receipt verify` does not disclose that to an operator. Amend the text or record an accepted divergence — do not read this row as "the stated mechanism shipped" |
