...
- [ ] **SEC-02**: Declarative sandboxed OAuth capture (`9b692e07`), capture-boundary hardening (`3c59c62e`), and the stdin fixture test (`d033c631`) are absorbed. **→ CARVED OUT to Phase 114 (2026-08-05, ROADMAP Amendment):** ~9 of `9b692e07`'s 26 files depend on subsystems absent from the fork (`oauth_capture/` core — 1,156 lines, `forward.rs`, `tls_intercept/{h2_forward,handle}.rs`); `forward.rs`'s `ResponseRewrite` hook is confirmed as the mechanism keeping real OAuth tokens out of the sandboxed client, so no reduced-scope absorb was attempted here. Reality-check evidence captured in `112-OAUTH-CAPTURE-DISPOSITION.md`. The adopt/adapt-down/decline disposition call belongs to Phase 114 — see `ROADMAP.md` § "Phase 114: OAuth Capture Absorb (SEC-02)". Not resolved by Phase 112; checkbox intentionally left unchecked, not silently dropped.
