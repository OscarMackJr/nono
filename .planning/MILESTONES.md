...
## v3.6 UPST12 Upstream Sync (v0.66.0 → v0.69.0) (Shipped: 2026-08-08)

**Phases completed:** 7 phases (108-114), 51 plans (343 commits since the v3.5-parallel start `cdffe6fb`; 87 source files, +21,333/−877 excl. `.planning/`; 268 files, +65,194/−957 all-in). Timeline 2026-07-28 → 2026-08-08 (11 days), run **in parallel** with the still-open v3.5.

**Delivered:** The fork is synced to upstream `nolabs-ai/nono` `v0.69.0` across the four-release `v0.66.0..v0.69.0` window without regressing the Windows security model or the ADR-86 policy-free-library boundary, with two large security subsystems (SPIFFE/SPIRE workload identity, declarative OAuth capture) absorbed under their own ADR-gated reviews, and the whole tree leapfrogged to a prepare-only `0.70.0`.

**Key accomplishments:**

- **UPST12 divergence audit (Phase 108):** A per-commit ledger over the full 100-commit window — 1,642 lines / 176 SHAs, every substantive commit classified adopt/adapt/skip/split with a `windows-touch` flag and per-cluster ADR verdict. Settled `deny_domain` posture in **ADR-108**. Fenced the `tool-sandbox/` subsystem (PR #1105, never absorbed) plus its 7 refinement PRs explicitly **DEFERRED→v3.7**. The audit's own measurement then invalidated its ROADMAP SC4: 27 hand-verified CODE commits mapped to none of v3.6's 12 requirements — surfaced with an exact count and resolved by proposing **Phase 112**, operator-approved rather than silently absorbed.
- **Proxy/network absorb (Phase 109):** `deny_domain` (#1374) wired into the filter and profile schema as defense-in-depth — checked strictly *before* the allowlist, never weakening default-deny — with a fail-closed guard proven live at **both** `nono run` entry points (D-04). `no_proxy` and `HTTP_PROXY` forward-proxy landed; #1430/#1437 confirmed non-applicable (target subsystems absent from the fork).
- **Profile/policy absorb (Phase 110):** `platform_overrides` per-OS patching preserved through `extends`; the fork's `windows_low_il_broker`/`windows_interpreters` sprawl migrated into it with back-compat aliases; `$VAR` + `@git:*` token expansion; bun/mise presets. **PROF-03's port-range schema shipped with a fork-original WFP-native emitter** and was proven against the live kernel on an elevated session: 8 filters from a verified-zero baseline, 4 `FWP_MATCH_RANGE` conditions at `49200..49210` — one per layer, never per-port unrolled — each `ALE_USER_ID`-scoped to the child and torn down to 0 on exit. Running that checkpoint surfaced 4 defects, all fixed, 3 proven live.
- **Core carry + release leapfrog (Phase 111):** macOS `~/.cache` grant and `MAX_CRYPTO_THREADS`=12; the `--memory`/`--max-processes` CLI surface reconciled onto the fork's existing kernel-enforced Job Object implementation as **ADAPT-not-adopt** (ADR-111) — no new enforcement — and stale help text falsely claiming pending Unix enforcement corrected. All 6 workspace crates + path-dep pins + both binding repos leapfrogged to `0.70.0`, collision-free above upstream's 0.69.0.
- **Security + residual sync (Phase 112):** Eight security requirements absorbed under a fork-invariant review kept separate from any release cut (the v3.1 Phase 87 precedent). NVIDIA procfs mediation, trust-policy `predicate` discriminator, Landlock `Refer` grant, seccomp supervisor-ancestry orphan reaping (adapted to the fork's own predicate), and the standalone `nono proxy` command. **SEC-08 was deliberately NOT absorbed** — upstream's `allow_vars` fix reverses the fork's own fail-closed default and would have reopened an env-var-leak class already closed (**ADR-112**, zero source change). **RUSTSEC-2026-0204 closed** (`crossbeam-epoch` 0.9.20; `cargo audit` clean).
- **SPIFFE/SPIRE workload identity (Phase 113):** `c831dade` — 4,354 insertions across 33 files, a refactor of fork-divergent code — absorbed **ADAPT-DOWN** per **ADR-113** after being split out of Phase 109 by operator decision. Direct JWT-SVID bearer injection and RFC 7523 jwt-bearer assertion, fail-closed on every non-SPIFFE path and on an unreachable SPIRE socket. ADR-86 boundary held: core `nono` carries only audit/telemetry types.
- **OAuth capture / SEC-02 (Phase 114):** Carved out of Phase 112 on operator decision after the plan-checker escalated it. Because ~9 of `9b692e07`'s 26 files depend on subsystems the fork does not have, a **fork-native** response buffer-and-rewrite enforcement point was built instead of importing upstream's absent `forward.rs`/`oauth_capture/` — fail-closed at all 8 non-success branches, wired into all three relay sites, with a zeroized session-scoped phantom store (upstream's `persist.rs` deliberately declined). Real tokens are rewritten to sandbox-visible phantoms before reaching the client.

**Audit outcome:** `passed_with_accepted_risk` — 23/23 requirements delivered, none unsatisfied, no orphans. The milestone audit itself found and forced a fix for **NEW-02**, a live path by which a `platform_overrides` block could silently drop a route's `capture`/`spiffe` on one OS and void the exact guarantee Phase 114 exists to provide (fixed `8b9fd74c`, field-by-field merge, regression test proven load-bearing). A second independent audit pass then found four more defects (NEW-05..08) plus three inaccuracies in the audit document itself, all one tier below the NEW-02 class and **carried to v3.7** alongside NEW-01 and NEW-03. Residual token-exposure paths (WR-02/03/05, NEW-04) are consciously accepted and recorded in `114-VERIFICATION.md` so a future reader can re-litigate them against the same reasoning.

**Process lesson of the milestone — executor Self-Check is not security evidence.** Phase 112's code-review gate caught **4 Critical fail-open defects the phase itself introduced** that all 8 executor self-checks had passed over. Phase 114 repeated the pattern twice: a route declaring both `spiffe` and `capture` still relayed the real token unrewritten while the executor reported PASSED (caught only by a hand control-flow audit), and the code-review gate then found 3 more Criticals in work that had already cleared every self-check *and* both cross-target clippy gates.

**Known deferred items at close:** 57 (43 historical quick-tasks, 6 dormant seeds SEED-001…006, 2 host-gated distribution todos, 2 partial UAT gaps in 111/112, 4 `human_needed` verifications — 111/112/113 operator-approved at their own gates, 101 belonging to the still-open v3.5) — all acknowledged, none v3.6 blockers (see STATE.md Deferred Items).

**Release scope = PREPARE ONLY.** `0.70.0` is prepared and the dry-run gate is GREEN; no tag push, no registry publish. Tagged **v3.6 LOCAL** — the tag was deliberately not pushed, consistent with every prior fork milestone and with v3.5's own paused operator tag-push decision.

**Archives:** [ROADMAP](milestones/v3.6-ROADMAP.md) · [REQUIREMENTS](milestones/v3.6-REQUIREMENTS.md) · [AUDIT](milestones/v3.6-MILESTONE-AUDIT.md)
