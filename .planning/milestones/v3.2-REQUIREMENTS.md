...
- [x] **VFY-01**: An override is accepted only when **both** the KMS signature verifies **and** a live ZT-Infra `POST /actions` lookup returns `allow` for the override `jti`/id (two-key AND gate; either alone is insufficient). _(Complete — live `POST /actions` AND-gate wired in Phase 93: `confined_run_checked` enforces `_live.live_check` before the Rust spawn. A redundant second live check in `_cli_apply.py` is a noted cosmetic warning, not a functional or security gap.)_
...
*Last updated: 2026-06-23 — Phase 93 (ZTL-01..05, CLI-01/02, DF-02 Complete) + VFY-01 closed (live POST /actions AND-gate wired; [BLOCKING-93] cleared). All 28 v1 requirements Complete.*
