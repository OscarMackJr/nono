...
- [x] **Phase 65: Minifilter ADR + macOS Live Re-validation** (completed 2026-06-11) - D-11c CI HARD gate ✅ GREEN; latency captured (SPAN medians 0.553/0.569 ms); **gate-65-A Seatbelt re-validation ✅ PASS on a real macOS host** (A1–A4; MACOS-03 satisfied). go/no-go ADR committed w/ recommendation (`Status: Proposed` — Oscar's Accepted-flip is the last v2.10 action). Finding: macOS resl enforcement (`--timeout`/`RLIMIT_NPROC`, A5) does NOT fire on a real host → filed as REQ-RESL-NIX-03 defect (separate from Seatbelt/MACOS-03)
- [x] **Phase 66: WR-02 EDR HUMAN-UAT** (completed 2026-06-11) - 10 assertions executed live on `nono-fltmgr-vm` (Sysmon v15.20 + Defender AV); **WR-02 CLOSED** (validated under a representative EDR-proxy). EDR-01/EDR-02 satisfied. Findings: confined child invisible to Sysmon telemetry; + 2 release-packaging todos (MSI VC++ prereq, untrusted POC cert → broker non-functional on clean host)
