...
- [x] **Phase 48: UPST6 sync execution** — Cherry-picks + D-20 manual replays per UPST6 audit dispositions; D-19 trailer convention + Windows-only-files invariant + baseline-aware CI gate inherited from Phase 22/34/40/43. (completed 2026-05-24 — 9 C4 cherry-picks `caab9967..e7da4998` on `pre-merge`; regression-free vs baseline `3f638dc6` after 3 CR-A cross-target fix rounds; remaining red CI lanes are pre-existing main-branch Class-B debt deferred to a CI-cleanup effort)
