...
gaps:
  - truth: "SC3 / CINT-03 — every entry in the contract has a test that forces that layer unavailable and asserts the contracted outcome; a contract row without a test is not counted as satisfied"
    ...
    reason: >
      ...
      `windows-layer-fault-injection` runs the feature legs — but CORRECTED 2026-08-15: as run that
      day (run 31888431118) that job was FAIL-OPEN and could not fail the build on them. Its two
      `cargo test` legs shared a single `run:` block with no `shell:` override, so under pwsh the
      step's status was its LAST native command's exit code; the passing `nono-shell-broker` leg
      masked the `nono-sandbox-cli` leg and the job reported `conclusion: success` over
      `test result: FAILED. 1684 passed; 22 failed`. Quick task 260815-f2x FIXED this by splitting
      the legs into one step per package — `Run layer-fault-injection suites (nono-sandbox-cli)` and
      `Run layer-fault-injection suites (nono-shell-broker)` — so each exit code is now checked
      independently. The SPEC's "or that test fails the build" claim is therefore backed by the
      WIRING as of that fix and by no executed build yet, not by an actual build as this pass
      recorded. Separately and STILL OPEN: that job is RED, with 22 failures whose triage is an
      agreed follow-up outside the scope of the wiring fix.
    ...
    missing:
      ...
      - "STILL OPEN: an elevated/CI Windows RUN of `non_owned_path_with_a_foreign_label_is_exempt_not_a_coverage_gap` to convert WR-20 from authored-but-unverified to verified. What is missing is a RUN — CORRECTED 2026-08-15: this bullet previously added `not CI wiring — no new CI wiring is required`, and the wiring half of that was false. See the correction later in this bullet: as run, the job could not fail on this test, and quick task 260815-f2x supplied the wiring that makes a RUN meaningful. The test is NOT feature-gated: it is a plain `#[test]` at `labels_guard.rs:1188` inside `#[cfg(test)] mod tests` (declared :567-570), so it lives in the `nono` bin's ordinary unit tests. CI job `windows-layer-fault-injection` therefore ALREADY runs it by construction: its cargo invocations carry no `--bin`/`--test`/`--lib` filter, so they run every target in the package. That COVERAGE half stands unchanged. What did NOT stand is the implied GATING value, CORRECTED 2026-08-15: as run that day (run 31888431118) the job was FAIL-OPEN — its two `cargo test` legs shared a single `run:` block with no `shell:` override, so under pwsh only the LAST native command's exit code became the step's status, the passing broker leg masked the `nono-sandbox-cli` leg, and the job reported `conclusion: success` over `test result: FAILED. 1684 passed; 22 failed`. A WR-20 failure in the `nono-sandbox-cli` leg would have been swallowed by exactly that mechanism, so a run of this job as-was could not have converted WR-20 from authored-but-unverified to verified. Quick task 260815-f2x FIXED the wiring by splitting the legs into `Run layer-fault-injection suites (nono-sandbox-cli)` and `Run layer-fault-injection suites (nono-shell-broker)`, one cargo command per step, so a WR-20 failure now fails the job. Separately and STILL OPEN: the job is RED with 22 failures whose triage is an agreed follow-up. The blocker is that the job has never executed this test — branch `milestone/v2.13-carryforward-closeout` is 684 commits ahead of origin (0 behind) and the last CI run on it, 2026-07-29, predates Phases 115/116/117 entirely. Pushing is an operator decision, which is why this stays a human item. Bounding the job's evidentiary value so it is not overstated: the `test` job matrix (`ci.yml:129-131`) is `[ubuntu-latest, macos-latest]`, making `windows-layer-fault-injection` the ONLY job that runs Rust unit tests on Windows anywhere in CI; and because it runs the whole package suite, the other 11 documented Windows-host baseline failures will surface there too. The 6 `config::tests` env races may well pass under its `--test-threads=1`, but that is a PREDICTION, NOT A MEASUREMENT. The override does not touch this — it is about SC3's ROW COVERAGE, not about that test, which continues to FAIL loudly per D-31 on this non-elevated host and keeps `cargo test --bin nono` RED at HEAD (1688 passed / 12 failed)."
...
human_verification:
  - test: "A RUN, not new CI wiring. Either (a) push branch `milestone/v2.13-carryforward-closeout` so CI job `windows-layer-fault-injection` executes it — the job already runs it BY CONSTRUCTION, because its cargo invocations carry no `--bin`/`--test`/`--lib` filter and therefore run every target in the package, and the test is NOT feature-gated (a plain `#[test]` at `labels_guard.rs:1188` inside `#[cfg(test)] mod tests` declared at :567-570, i.e. one of the `nono` bin's ordinary unit tests) — but route (a) is only informative from the post-2026-08-15 wiring onward, because the job was FAIL-OPEN as run that day and would have reported success regardless of this test's result (see `why_human`); or (b) run `cargo test -p nono-sandbox-cli --bin nono non_owned_path_with_a_foreign_label_is_exempt_not_a_coverage_gap` from an ELEVATED session that holds SeTakeOwnershipPrivilege / SeRestorePrivilege / SeBackupPrivilege."
    ...
    why_human: "CORRECTED 2026-08-15: this item previously opened `No new CI wiring is required`. The COVERAGE half of that was and is true — the unfiltered cargo invocations do run this test. The GATING half was false: on run 31888431118 the job held both `cargo test` legs in one `run:` block with no `shell:` override, so under pwsh only the LAST native command's exit code became the step's status; the passing `nono-shell-broker` leg masked the `nono-sandbox-cli` leg, and the job reported `conclusion: success` over `test result: FAILED. 1684 passed; 22 failed`. A WR-20 failure lives in the masked leg, so the job could not have converted WR-20 from authored-but-unverified to verified. Wiring WAS therefore required, and quick task 260815-f2x supplied it — the legs are now `Run layer-fault-injection suites (nono-sandbox-cli)` and `Run layer-fault-injection suites (nono-shell-broker)`, one cargo command per step (SPEC D-15 ledger row `CI-01`). STILL OPEN separately: the job is RED with those 22 failures, triage is an agreed follow-up. With that fix in place the coverage exists and what is missing is a RUN, which is blocked on an operator decision to push. Branch `milestone/v2.13-carryforward-closeout` is 684 commits ahead of origin (0 behind), and the last CI run on it — 2026-07-29 — predates Phases 115/116/117 entirely, so `windows-layer-fault-injection` has never executed this test. It also cannot be closed locally: this dev host runs a non-elevated domain account; `icacls /setowner` fails for both SYSTEM and BUILTIN\\Administrators. The test fails LOUDLY with a full explanatory message per D-31 rather than skipping, so the gap is disclosed — but it cannot be closed from this host, and WR-20 stays authored-but-unverified until a real run exists, with `cargo test --bin nono` RED at HEAD (1688 passed / 12 failed). Bounding the job's evidentiary value so it is not overstated: the `test` job matrix (`ci.yml:129-131`) is `[ubuntu-latest, macos-latest]`, making `windows-layer-fault-injection` the ONLY job that runs Rust unit tests on Windows anywhere in CI; and because it runs the whole package suite it will also inherit the other 11 documented Windows-host baseline failures. The 6 `config::tests` env races may well pass under its `--test-threads=1`, but that is a PREDICTION, NOT A MEASUREMENT."
...
| `.github/workflows/ci.yml` | Gate that actually runs the fault-injection suites | ⚠ CORRECTED 2026-08-15 — ran the suites, but was FAIL-OPEN as run; wiring now fixed, job still RED | `windows-layer-fault-injection` builds `nono-shell-broker --release` then runs both feature legs with `--test-threads=1`. **This cell read `✓ VERIFIED` at this pass and was wrong about what the gate could DO.** As run on 2026-08-15 (run 31888431118) the two `cargo test` legs shared one `run:` block with no `shell:` override, so under pwsh the step's status was its LAST native command's exit code: the passing broker leg masked the `nono-sandbox-cli` leg and the job reported `conclusion: success` over `test result: FAILED. 1684 passed; 22 failed`. The gate ran the suites but could not fail on them. Quick task 260815-f2x split it into `Run layer-fault-injection suites (nono-sandbox-cli)` and `Run layer-fault-injection suites (nono-shell-broker)` — and the `windows-packaging` step, which carried the identical latent shape, into `Build release binary: nono-sandbox-cli` and `Build release binary: nono-shell-broker` — so each exit code is checked. Recorded in the SPEC's D-15 ledger as `CI-01`. Separately and STILL OPEN: the job is RED with those 22 failures, triage is an agreed follow-up. |
...
1. **A RUN of the WR-20 pin — not CI wiring** — `non_owned_path_with_a_foreign_label_is_exempt_not_a_coverage_gap`
   ...
   exists. **CORRECTED 2026-08-15:** this item previously continued *"and no new CI wiring is
   required"*; wiring WAS required, and the correction is stated in full below. The test is NOT
   feature-gated — it is a plain
   ...
   already runs it **by construction**: its cargo invocations carry no `--bin`/`--test`/`--lib`
   filter, so they run every target in the package. That coverage claim is unchanged.
   **CORRECTED 2026-08-15 — the job as run could not have FAILED on it.** On run 31888431118 the
   step then named `Run layer-fault-injection suites` held both `cargo test` legs in one `run:`
   block with no `shell:` override; under pwsh a step's status is its LAST native command's exit
   code, so the passing `nono-shell-broker` leg masked the `nono-sandbox-cli` leg and the job
   reported `conclusion: success` over `test result: FAILED. 1684 passed; 22 failed`. A WR-20
   failure sits in the `nono-sandbox-cli` leg, so a run of the job as-was would have reported
   success regardless of WR-20's result — it could not have converted WR-20 from
   authored-but-unverified to verified. Quick task 260815-f2x FIXED the wiring: the legs are now
   `Run layer-fault-injection suites (nono-sandbox-cli)` and
   `Run layer-fault-injection suites (nono-shell-broker)`, one cargo command per step, so each exit
   code is checked. Recorded in the SPEC's D-15 ledger as `CI-01`. **STILL OPEN, separately:** the
   job is RED with those 22 failures, whose triage is an agreed follow-up and is not this fix.
   What is missing is a RUN **of the fixed wiring**. The branch is **684 commits ahead of origin** (0 behind) and the last
