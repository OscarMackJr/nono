...
- **D-10:** No audit-ledger emissions in Phase 31. Broker spawn / Low-IL child creation logged via `tracing` (`info!`/`debug!`) only. AuditEventPayload extension for shell launches stays out of scope — would expand Phase 23's wire-shape and force a new RejectStage-equivalent design call. v2.4 follow-up: "Wire shell-launch events into audit ledger for parity with AIPC capability_decision emissions."
- **D-11:** AppliedLabelsGuard Drop-ordering bug (Phase 30 D-09 — 9 leaked Low-IL labels observed on test box) stays as separate quick task `nono-labels-guard-leak`. Phase 31 is the broker lift; mixing in a Drop-lifecycle bug in `AppliedLabelsGuard` expands scope unrelated to the broker pattern. Phase 31 field-test treats label warnings as expected; not a Phase 31 failure indicator.
