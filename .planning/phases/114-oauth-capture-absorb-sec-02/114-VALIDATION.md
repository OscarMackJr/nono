...
| Non-admitted or unknown phantom in a request_nonce_fields path is left unchanged (never resolved to a real token) | D-02r / D-06 (revision fix) | Elevation of Privilege | Fail closed **on substitution**, not on the request — no fallthrough to a real token for a consumer the phantom was not admitted to; the request is still forwarded and upstream rejects the phantom | integration | `cargo test -p nono-proxy capture_egress_never_substitutes_real_token_for_unadmitted_consumer` | ❌ W0 | ⬜ pending |
| The resolve call actually **fires with correct arguments on a live dispatch path** — the REAL token, not the phantom, is what reaches upstream | D-02r (checker iter-2 warning) | Info Disclosure / half-feature | End-to-end through `handle_reverse_proxy` against a fake upstream; a grep proves the call is *written*, this proves it *works* | integration | `cargo test -p nono-proxy capture_egress_resolution_reaches_upstream_on_live_dispatch_path` | ❌ W0 | ⬜ pending |
...
- [ ] `resolve_capture_request_body` tests in `reverse.rs` (Plan 114-06 Task 3, revision fix) —
      **3 cases**: admitted-phantom-resolves-in-request-body,
      unadmitted-or-unknown-phantom-left-unchanged, and the end-to-end
      `capture_egress_resolution_reaches_upstream_on_live_dispatch_path` driving a live request
      through `handle_reverse_proxy` against a fake upstream. The first two prove the resolution
      logic; the third proves the call actually fires with correct arguments. Together they close
      the plan-checker's SC2 half-feature blocker (iteration 1) and its wiring-rigor warning
      (iteration 2). Reuse the existing `TcpListener` fixture pattern in `reverse.rs`'s `mod
      tests` — do not invent a second fixture style.
...
| Baseline test-failure diff | SC4 | Host has documented pre-existing failures | Diff the workspace suite against the documented inherited failing baseline. **Authoritative source: `.planning/STATE.md`'s Phase 113 entry (11 named failures)** — the older "4 `nono-cli` + 1 `nono` lib" figure is stale; Plan 114-11 Task 3 supersedes it. Do **not** chase these as regressions. |
