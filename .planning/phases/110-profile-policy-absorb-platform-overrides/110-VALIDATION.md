...
|-----|-----------------|------------|-----------|-------------------|--------------|
| PROF-01 | `platform_overrides` parses; applies current-OS block only; survives `extends` resolution; `windows_low_il_broker`/`windows_interpreters` accepted in BOTH forms with fail-secure OR/union semantics preserved for those two flags (D-08a, narrowing D-08's new-form-wins to ordinary settings only) | override silently widening scope (EoP) | unit | `cargo test -p nono-cli profile::mod::tests -- platform_overrides` | ✅ Closed by Plan 110-01 Task 3 — ports upstream's `platform_overrides_*` suite from `ae1c513e`/`719975cf` (~8 tests) plus 5 new tests: 2 D-08 back-compat, 1 D-08a OR-semantics distinguishing proof, 1 `windows_interpreters` union proof, 1 schema-resolution proof. Phase 51's `merge_profiles_or_semantics_base_true_child_false` test verified to remain present/unmodified. |
...
| PROF-03c | Linux Landlock adds `NetPort` rules per range, no cap (full 1–65535) | — | unit | `cargo test -p nono sandbox::linux::tests -- port_range` | ✅ Closed by Plan 110-03 Task 3 — original test coverage added (no upstream precedent; upstream extended the ruleset loop but added no dedicated `linux.rs` unit test) |
| PROF-03d | Windows filter-spec construction emits a `FWP_MATCH_RANGE` condition per range with **no per-port unroll** and no macOS-style cap | `valueLow`/`valueHigh` inversion widening the matched span (EoP) | unit | `cargo test` against the `nono-wfp-service` spec-builder test module | ✅ Closed by Plan 110-06 Task 2 — fresh unit tests for the new `PortCondition::RemoteRange`/`LocalRange` spec-builder construction (fork-original, D-07; no upstream precedent), following the existing `localhost_ports: vec![8080]` fixture pattern at `nono-wfp-service.rs:1829-1936` |
| PROF-03e | Live kernel `FwpmFilterAdd0` accepts a range filter **and it is actually enforced** | — | manual / host-gated | none automatable from this dev host | N/A — requires Administrator + live `nono-wfp-service`; recorded as `checkpoint:human-verify` in Plan 110-06/110-08 |
...
| PROF-04 | `bun`/`mise` presets are schema-valid **and resolvable** — loaded and resolved *by name*, not merely present in JSON (D-11) | — | unit | `cargo test -p nono-cli -- manifest_roundtrip` + a new "resolve `bun-dev`/`mise-dev` by name" test | ✅ Closed by Plan 110-07 Task 2 — explicit "resolve by name" test, distinct from `test_schema_validates_builtin_profiles_in_policy_json`'s schema-conformance-only coverage |

---

## Wave 0 Requirements

Test infrastructure itself is fully present — no test-runner or fixture install needed. All 4 gaps
below are assigned to a closing plan/task (verified against the current 8-plan set 2026-07-30):

- [x] `crates/nono/src/sandbox/linux.rs` — no existing `#[test]` coverage for the new port-range Landlock rules — **closed by Plan 110-03 Task 3** (PROF-03c)
- [x] `crates/nono-cli/src/bin/nono-wfp-service.rs` — new `PortCondition::RemoteRange`/`LocalRange` construction needs fresh unit tests, fork-original, no upstream precedent — **closed by Plan 110-06 Task 2** (PROF-03d)
- [x] `crates/nono-cli/src/profile/mod.rs` (or a schema-test module) — explicit schema-validation-**plus-resolution** tests for `platform_overrides` / `open_port_range` / `listen_port_range`, beyond the built-in-profiles iteration test — **closed by Plan 110-01 Task 3** (`platform_overrides`) **and Plan 110-04 Task 1** (`open_port_range`/`listen_port_range`)
- [x] `crates/nono-cli/src/profile/mod.rs` — "resolve `bun-dev`/`mise-dev` by name" tests, distinct from schema-shape validation (PROF-04, D-11) — **closed by Plan 110-07 Task 2**
...
- [x] Wave 0 covers all 4 MISSING references above (all 4 now closed — see table above)
- [x] No watch-mode flags
- [ ] Feedback latency < 90s for the quick run — execution-time fact (actual measured latency), verified during execution, not planning
- [ ] Both cross-target clippy gates green (D-12) — execution-time fact, verified during execution, not planning
- [x] `nyquist_compliant: true` set in frontmatter

**Approval:** 2026-07-30 (revision iteration 1 — Blockers 1-3 and Warnings 1-2 from plan-checker addressed; see 110-01/110-03/110-04-PLAN.md and 110-RESEARCH.md for the corrected content)
