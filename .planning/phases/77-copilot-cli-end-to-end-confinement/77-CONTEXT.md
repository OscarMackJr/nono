...
- **D-05:** Rely on the carried-forward fact that the **AppContainer package SID is derived from the profile name and is therefore stable across runs** — this is what makes a *one-time* admin grant durable. Researcher must re-confirm SID stability for the standalone-Copilot profile before locking the setup step.
