...
if (Test-Path $dst) {
    ...
    if ($existing -gt 5GB) { Write-Output "Already downloaded (over 5 GB). Skipping. Delete C:\ewdk\ewdk.iso to force re-download."; return }
...
if (-not (Test-Path $dst)) { Write-Output ("DOWNLOAD FAILED (curl exit {0}). No file produced." -f $code); return }
$len = (Get-Item $dst).Length
Write-Output ("curl exit: {0} | size: {1:N2} GB | elapsed: {2:N1} min" -f $code, ($len/1GB), $sw.Elapsed.TotalMinutes)
if ($len -lt 5GB) {
    Write-Output "WARNING: file under 5 GB. Likely the license HTML page, not the ISO. First lines:"
