...
- **D-02 (egress target): ~~non-loopback mock bind~~ → OVERRIDDEN to loopback `127.0.0.1` (2026-06-18, post-research).** The original D-02 locked a non-loopback bind on the *assumption* that AppContainer+WFP might exempt loopback. D-02 explicitly conditioned this on researcher validation, and the research (79-RESEARCH.md "D-02 Answer") **falsified that assumption**: the AppContainer loopback restriction is *receive-side* (AppContainer↔AppContainer only); the mock server runs Medium-IL in the gate process, so Agent A (AppContainer client) reaches `127.0.0.1` normally, while Agent B's deny still fires at the SID-keyed **connect** layer (`FWPM_LAYER_ALE_AUTH_CONNECT_V4`) — a real, attributable block. The non-loopback path was found likely-**infeasible in scope** (reaching a LAN IPv4 from an AppContainer needs the `privateNetworkClientServer` capability, which `nono-profile.schema.json` does NOT expose). **DECISION (user, 2026-06-18): bind the mock server to `127.0.0.1:0` in the Medium-IL gate process.** The MEDIUM-confidence "receive-side only" claim (Project Zero 2021) is accepted because the gate *empirically* proves A-allowed/B-denied on the live Win11 host every run — a silently-exempt target would surface as B also succeeding (FAIL), not a false PASS.
