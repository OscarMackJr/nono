...
### Research Resolutions (2026-06-21, post-RESEARCH.md, operator-confirmed)
- **D-05 (Crypto primitive correction — SUPERSEDES the `verify_keyed_signature` reference in D-01/canonical_refs):** RESEARCH.md found `nono::trust::verify_keyed_signature` (`bundle.rs:463`) is a **Sigstore DSSE/PAE** verifier (computes PAE over an in-toto payload), NOT raw ECDSA over a SHA-256 digest. ZT-Infra/KMS signs the **raw 32-byte digest**. The verifier MUST instead call **`sigstore_verify::crypto::...VerificationKey::verify_prehashed(&Sha256Hash, &SignatureBytes)`** (sigstore-crypto 0.8.0, already a transitive dep, aws-lc-rs-backed). This remains a **raw-key** path — NOT sigstore-keyless/OIDC (honors the SEED-005 lock). **Low-S is NOT auto-enforced** by aws-lc-rs ASN.1 verify — VFY-02 requires an **explicit low-S DER normalization/reject** mirroring ZT's `audit.js::normalizeEcdsaDerLowS` / `signatures.py::_require_low_s`.
- **D-06 (Override-token schema is a nono-side construct — resolves OQ-1):** ZT-Infra defines **no** override-token schema (only CAF AuditRecords + registry entries). The OVR-01 token fields (scope, `not_before`, `expires_at`, `jti`, repo_context, algorithm, signature, key-ARN) are a **nono-side construct** carried inside the signature-covered canonical object. Phase 91 locks a **fail-secure serde model now** (`deny_unknown_fields`), signed by the **local test keypair** (D-01) for offline tests. The plan MUST carry an explicit **`[BLOCKING]`-flagged note** that this wire shape must be reconciled with real KMS-issued tokens before Phase 93's live `POST /actions` arm — it is NOT a gap, it is a deliberate v1 boundary.
