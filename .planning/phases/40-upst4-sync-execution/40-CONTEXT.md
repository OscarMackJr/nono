...
- **D-40-E1 (= Phase 22 D-17 / Phase 34 D-34-E1): Windows-only files structurally invariant.** Any cherry-pick or manual port that touches `*_windows.rs` files or `crates/nono-cli/src/exec_strategy_windows/` subtree is by definition a cherry-pick bug — abort, investigate, revert the Windows hunk. The Phase 24 D-11 drift-tool filter ensures these never appear in upstream commits we cherry-pick. Manual ports must explicitly diff-check `git diff --stat HEAD~1 HEAD -- crates/ | grep -E '_windows|exec_strategy_windows'` and confirm zero hits.
  ...
  - **D-40-E1 addendum (added mid-execution 2026-05-14): forced fork-adaptation exception.** When an upstream cherry-pick introduces a **required new field** on a *cross-platform* struct (i.e. one whose definition is NOT inside a `#[cfg(target_os = ...)]` block) and that struct is constructed by Windows-side wiring code, the Windows-side construction call site must add the new field to compile. Such an edit is permitted iff ALL of the following hold:
    1. The cross-platform struct is non-optional / required (no `Default` or `Option` escape).
    2. The Windows-side edit uses ONLY a documented cross-platform default factory (e.g. `nono::ScrubPolicy::secure_default()`) — no Windows-specific behavior, value, or branch.
    3. The diff in `*_windows.rs` is minimal (≤ ~5 lines) and adds NO new public API, NO new control flow, and NO `#[cfg(target_os = "windows")]`-specific scrub/credential/path logic (D-40-E6 still binding).
    4. The deviation is documented in the plan's `SUMMARY.md` under an explicit `## Deviations from Plan` section AND mirrored to STATE.md so it cannot be silently absorbed.
  Precedent: Plan 40-03 commit `96886ae9` (+4 lines in `crates/nono-cli/src/exec_strategy_windows/mod.rs` wiring `RollbackExitContext.redaction_policy: &ScrubPolicy` with `secure_default()`). Reviewed and accepted post-hoc on 2026-05-14; documented in `.planning/phases/40-upst4-sync-execution/40-03-SCRUB-MODULE-SUMMARY.md`. The general rule (abort + revert) remains the default; this exception is intentionally narrow and applies only when the structural-field forcing function is unavoidable.
